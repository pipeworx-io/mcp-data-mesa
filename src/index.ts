...
async function pwFetch(url: string | URL, init?: RequestInit): Promise<Response> {
  return fetchWithTimeout(url, init ?? {}, 'Mesa Open Data');
}

const BASE = 'https://citydata.mesaaz.gov';
const UA = 'pipeworx-mcp-data-mesa/1.0 (+https://pipeworx.io)';

const tools: McpToolExport['tools'] = [
  {
    name: 'datasets',
    description: 'Search the Mesa Open Data catalog of open datasets by keyword. Returns each dataset\'s resource_id, name, description, category and update date — pass the resource_id to query/metadata.',
    inputSchema: {
      type: 'object',
      properties: {
  ...
  {
    name: 'query',
    description: 'Run a Socrata SoQL query against a Mesa Open Data dataset by resource_id (e.g. "2gkz-7z4f"). Filter with where/select/group/order (SoQL clauses, without the leading $) plus limit/offset. Returns matching rows as JSON.',
    inputSchema: {
      type: 'object',
      properties: {
        resource_id: { type: 'string', description: 'Dataset id, e.g. "2gkz-7z4f" (from datasets).' },
  ...
  {
    name: 'metadata',
    description: 'Get a Mesa Open Data dataset\'s schema + metadata (columns, types, row count, category, last-updated) by resource_id, e.g. "2gkz-7z4f".',
    inputSchema: {
      type: 'object',
      properties: { resource_id: { type: 'string', description: 'Dataset id, e.g. "2gkz-7z4f".' } },
      required: ['resource_id'],
    },
  },
];
...
async function callTool(name: string, args: Record<string, unknown>): Promise<unknown> {
  switch (name) {
    ...
    case 'query': {
      const id = reqStr(args, 'resource_id', '"2gkz-7z4f"');
      const p = new URLSearchParams();
    ...
    case 'metadata':
      return socrataGet(`/api/views/${reqStr(args, 'resource_id', '"2gkz-7z4f"')}.json`);
    default:
...
async function socrataGet(path: string): Promise<unknown> {
  ...
  if (!res.ok) throw await httpError(res, 'citydata.mesaaz.gov');
  return res.json();
}
